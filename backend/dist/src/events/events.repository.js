"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EventsRepository = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const event_schema_1 = require("./schemas/event.schema");
let EventsRepository = class EventsRepository {
    eventModel;
    constructor(eventModel) {
        this.eventModel = eventModel;
    }
    async create(eventData) {
        const event = new this.eventModel(eventData);
        return event.save();
    }
    async findById(id) {
        return this.eventModel.findById(id).exec();
    }
    async findByCompany(companyId) {
        const objectId = typeof companyId === 'string' ? new mongoose_2.Types.ObjectId(companyId) : companyId;
        return this.eventModel
            .find({ companyId: objectId })
            .sort({ dateCreated: -1 })
            .exec();
    }
    async findByVendor(vendorId) {
        const objectId = typeof vendorId === 'string' ? new mongoose_2.Types.ObjectId(vendorId) : vendorId;
        return this.eventModel
            .find({ vendorId: objectId })
            .sort({ dateCreated: -1 })
            .exec();
    }
    async update(id, updateData) {
        return this.eventModel
            .findByIdAndUpdate(id, { ...updateData, lastModified: new Date() }, { new: true })
            .exec();
    }
};
exports.EventsRepository = EventsRepository;
exports.EventsRepository = EventsRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(event_schema_1.Event.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], EventsRepository);
//# sourceMappingURL=events.repository.js.map