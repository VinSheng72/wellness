...
/**
 * HR Admin Dashboard - Server Component
 * Fetches events for the authenticated user's company
...
export default async function HRDashboardPage() {
  ...
  if (!userInfo || userInfo.role !== 'HR_ADMIN' || !userInfo.companyId) {
    redirect('/login');
  }

  // Fetch company information
  const company = await CompanyRepository.findById(userInfo.companyId);
  ...
  const events = await EventService.getEventsByCompany(userInfo.companyId);
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-600">
              {company.name} | {userInfo.username}
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="space-y-8">
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            ...
            <EventTable events={serializedEvents} userRole="HR_ADMIN" />
          </div>
        </div>
      </main>
    </div>
